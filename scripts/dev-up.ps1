...
for ($attempt = 1; $attempt -le 36; $attempt++) {
  try { $response = Invoke-WebRequest -Uri "http://localhost:3000/api/health" -UseBasicParsing -TimeoutSec 3; if ($response.StatusCode -eq 200) { Write-Host "DojoNexo está disponible en http://localhost:3000" -ForegroundColor Green; exit 0 } } catch { Start-Sleep -Seconds 5 }
}
docker compose logs app
throw "DojoNexo no alcanzó un estado saludable dentro del tiempo esperado."
